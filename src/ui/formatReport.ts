...
export function formatShortReport(result: VerificationResult): string {
  ...
  lines.push("");
  lines.push("This report does not prove truth, fairness, legality, correctness, or accountability.");
...
export function formatMarkdownReport(
  ...
  if (mergedOptions.includeTechnicalDetails) {
    ...
    lines.push(`- Modules run: ${result.modules_run.join(", ") || "None reported"}`);
    lines.push(
      `- Modules skipped: ${result.modules_skipped.join(", ") || "None reported"}`
    );
  }
  ...
  if (mergedOptions.includeMachineReadableHint) {
    ...
    lines.push("");
    lines.push("A machine-readable JSON result should be exported separately from this report.");
  }

  lines.push("");
  lines.push("## Final Note");
  lines.push("");
  lines.push("This report helps test the record. It is not a legal, fairness, or accountability determination.");
