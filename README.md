...
- reproducible dependency classification outside the UI;
- minimal JSON expansion;
- no claim of truth, fairness, legality, correctness, independence, or full accountability.
...
The answer is to make the verifier’s own classification reproducible outside the app.
...
The goal is to make operator dependence impossible to hide without letting the verifier certify its own inference.
...
8. Keep JSON expansion minimal.
9. Avoid implying bad intent from dependency detection.
10. Keep full NDC computation for future work.
...
The JSON result should remain a verification artifact, not a legal determination and not proof of independence.
...
The goal is to make the app’s dependency classification reproducible outside the app.
...
Those labels require full dependency analysis or external reproducibility beyond this MVP.
...
> an incremental MVP improvement for citizen-facing verification of committed decision records, including minimal custody provenance, conservative dependency detection, and reproducible dependency classification rules.
...
- make dependency classification reproducible outside the UI through deterministic rules.
...
> A-DAP supports reconstructible verification conditions. It does not, by itself, prove truth, fairness, legality, correctness, independence, or accountability.
...
- no production-grade, legal, fairness, independence, or accountability claim is introduced;
...
- the same input materials can be processed by a separate script or CLI to reproduce the same dependency classification;
...
>
> Make operator dependence visible, and make the dependency classification reproducible outside the app.
